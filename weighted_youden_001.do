...
**-----------------------------------------------------------------------
somersd diabkfpg phat [pweight=wfinal1_ad] if sex==1, transf(z) cluster(ed) cimatrix(cil)
matrix b = e(b)
local yi = b[1,1] 
local yi_l`x' = cil[1,2]
local yi_u`x' = cil[1,3]
di   "Area under the Curve: " %4.1f `yi`x'' "  (95% CI " %4.1f `yi_l`x'' ", " %4.1f `yi_u`x'' ")"
...
postfile diabyi sex tot1 tot2 bmi yi yil yiu using `diab_yi'
	...
	** Loop BMI in increments of 1 between 20 and 35
	...
	qui {
		...
		** is/are quantitative predictors for that disease, then Harrell’s c is the area under the ROC curve	
		somersd diabkfpg bmi`x' [pweight=wfinal1_ad] if sex==`s', transf(z) cluster(ed) cimatrix(cil`x')
		** Extract matrix elements and post these elements to the temporary Stata file
		matrix b`x' = e(b)
		local yi`x' = b`x'[1,1] 
		local yi_l`x' = cil`x'[1,2]
		local yi_u`x' = cil`x'[1,3]
		di   "Area under the Curve: " %4.1f `yi`x'' "  (95% CI " %4.1f `yi_l`x'' ", " %4.1f `yi_u`x'' ")"
		post diabyi (`s') (`num1') (`num2') (`bmi') (`yi`x'') (`yi_l`x'') (`yi_u`x'')
...
preserve
	use `diab_yi', clear
	table bmi,  by(sex) c(mean tot1 mean tot2 mean yi mean yil mean yiu) format(%9.1f)

**-----------------------------------------------------------------------
** (07)
** Visual comparison of Youden indices (with 95% CIs)
** Investigating difference by looking at CI overlap
**-----------------------------------------------------------------------
#delimit ;
	gr twoway 
		/// Coverage by wealth
		(rspike yil yiu bmi if sex==1, hor lc(gs6) lw(0.25))
		(sc bmi yi if sex==1,  msize(4) m(o) mlc(gs0) mfc("253 174 97") mlw(0.1))
		,
			graphregion(color(gs16)) ysize(5)

			xlab(, labs(4) nogrid glc(gs16))
			xscale(fill ) 
			xtitle("Youden Index with 95% CI", size(4) margin(l=2 r=2 t=5 b=2)) 
			///xmtick(40(1)70)
...
**-----------------------------------------------------------------------
somersd diabkfpg bmi20-bmi35 [pweight=wfinal1_ad] if sex==1, transf(z) cluster(ed) cimatrix(cid)
